...
import numpy as np
import pandas as pd
from ..match import DualDTWNoteMatcher
...
def match_midis(
    ...
    # rename fields
    performance_midi_na = rfn.rename_fields(
        performance_midi_na,
        {field: f"performance_{field}" for field in performance_midi_na.dtype.names},
    )
    ref_midi_na = rfn.rename_fields(
        ref_midi_na, {field: f"ref_{field}" for field in ref_midi_na.dtype.names}
    )
    ...
    map_performance_id_to_idx = {
        note["performance_id"]: idx for idx, note in enumerate(performance_na)
    }
    ...
    for idx, align_entry in enumerate(pred_alignment):
        if align_entry["label"] == "match":
            ...
            performance_na_idx = map_performance_id_to_idx[
                align_entry["performance_id"]
            ]
            ...
            meta_columns.append(
                (
                    idx,
                    "match",
                    performance_na["performance_onset_sec"][performance_na_idx]
                    - ref_na["ref_onset_sec"][ref_na_idx],
                )
            )
    ...
    ref_na_full = rfn.append_fields(
        ref_na, names="match_number", data=ref_match_number, dtypes="i4", usemask=False
    )
    ...
    dt = np.dtype(
        [("match_number", "i4"), ("alignment_type", "U256"), ("timing_deviation", "f4")]
    )
    ...
    merged_df = df1.merge(df2, on="match_number", how="outer").merge(
        df3, on="match_number", how="outer"
    )
    output_df = merged_df.loc[
        merged_df.ref_onset_sec.fillna(merged_df.performance_onset_sec).argsort()
    ].drop(columns=["match_number"])
